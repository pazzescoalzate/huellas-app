/* HUELLA — Perfil: mapa de huellas interactivo + sellos ilustrados */
...
import { CAT, PROFILE, byId } from "../data/huella.js";
  ...
  return (
    <button onClick={(e) => { e.stopPropagation(); onClick(); }}
      ...
      <span className="w-[34px] h-[34px] grid place-items-center"
        style={{ borderRadius: "50% 50% 50% 4px", background: grad,
      ...
      {sel && (
        <div className="glass absolute left-3 right-3 bottom-3 rounded-lg p-[11px] flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[12px] font-light text-ink-soft mt-px flex items-center gap-[5px]">
              {sel.place} · <Icon name="star" size={11} color="var(--arena)" fill="var(--arena)" stroke={0} /> {sel.rating}
            </div>
          </div>
  ...
  return (
    <div className="flex items-center gap-3 py-[9px]">
      ...
      <div className="flex gap-px">
        {[0,1,2,3,4].map((i) => <Icon key={i} name="star" size={12} color="var(--arena)" fill="var(--arena)" stroke={0} />)}
      </div>
    </div>
  );
}

export default function ProfileScreen({ prefs, onSavePrefs }) {
  const p = PROFILE;
  const [settings, setSettings] = useState(false);
  const [allBadges, setAllBadges] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const visitedExps = p.visited.map((id) => byId(id)).filter(Boolean);
  const earned = p.badges.filter((b) => b.got);
  const locked = p.badges.filter((b) => !b.got);
  return (
    <div onScroll={(e) => setScrolled(e.target.scrollTop > 8)} className="flex-1 overflow-y-auto pb-24">
      <div className="sticky top-0 z-20 flex items-center justify-between px-5 py-2.5"
        ...
        <div className="text-[17px] font-semibold text-ink-strong tracking-[-0.01em]" style={{ opacity: scrolled ? 1 : 0 }}>{p.name}</div>
      ...
      <div className="flex flex-col items-center pt-2 px-[22px] pb-5">
        <div className="relative w-[86px] h-[86px]">
          <div className="absolute inset-0 rounded-full grid place-items-center text-white text-[32px] font-semibold border-2 border-white/[0.16] shadow-elev2"
            style={{ background: "linear-gradient(135deg, #C05A34, #7A3E55)" }}>{p.initials}</div>
          <PhotoSlot id="avatar-photo" shape="circle"
            style={{ position: "absolute", inset: 0, width: 86, height: 86, zIndex: 2 }} />
          <span className="absolute -right-0.5 bottom-0.5 w-7 h-7 rounded-full grid place-items-center bg-accent z-[3] pointer-events-none" style={{ border: "2px solid var(--bg-1)" }}>
            <Icon name="camera" size={14} color="var(--on-accent)" stroke={2} />
          </span>
        </div>
        <div className="text-[22px] font-semibold text-ink-strong tracking-[-0.01em] mt-3.5">{p.name}</div>
        <div className="text-[13px] font-light text-ink-faint mt-0.5">{p.handle} · Explorando desde {p.since}</div>
      </div>

      <div className="mx-[22px] px-3.5 py-4 rounded-lg flex bg-white/[0.045] border border-cardstroke">
        <StatCell value={p.stats.lugares} label="Lugares" />
        <div className="w-px bg-cardstroke"></div>
        <StatCell value={p.stats.ciudades} label="Ciudades" />
        <div className="w-px bg-cardstroke"></div>
        <StatCell value={p.stats.tours} label="Tours" />
        <div className="w-px bg-cardstroke"></div>
        <StatCell value={p.stats.reviews} label="Reseñas" />
      </div>
      ...
      <div className="pt-7">
        <div className="flex items-center justify-between mx-[22px] mb-4">
          <h2 className="text-[18px] font-semibold text-ink-strong">
            Sellos <span className="text-[13px] font-normal text-ink-faint ml-1.5">{earned.length} de {p.badges.length}</span>
          </h2>
          <button onClick={() => setAllBadges(true)} className="text-[13px] font-medium text-accent-soft inline-flex items-center gap-[3px]">
        ...
        <div className="flex gap-3 overflow-x-auto px-[22px] pb-1 snap-x snap-mandatory">
          {earned.map((b) => <div key={b.id} className="snap-start"><Patch b={b} /></div>)}
          <button onClick={() => setAllBadges(true)} className="w-[130px] shrink-0 self-stretch flex flex-col items-center justify-center gap-2 rounded-lg border-[1.5px] border-dashed border-ink-ghost my-1 mb-[30px]">
      ...
      <div className="pt-7 px-[22px]">
        <h2 className="text-[18px] font-semibold text-ink-strong mb-1.5">Visitados recientemente</h2>
        <div>
      ...
      {settings && <SettingsSheet prefs={prefs} onSave={onSavePrefs} onClose={() => setSettings(false)} />}
      {allBadges && <BadgesSheet badges={p.badges} onClose={() => setAllBadges(false)} />}
    </div>
  );
}
    ...
    <div className="absolute inset-0 z-[70] flex flex-col">
      ...
      <div className={"bdg-sheet absolute left-0 right-0 bottom-0 max-h-[88%] bg-bg2 rounded-t-xl flex flex-col shadow-[0_-20px_60px_rgba(0,0,0,0.5)] border-t border-glassstroke " + (closing ? "bdg-sheet-out" : "bdg-sheet-in")}>
        <div className="pt-2.5 px-[22px] pb-2 shrink-0">
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-[21px] font-semibold text-ink-strong tracking-[-0.01em]">
              Tus sellos <span className="text-[14px] font-normal text-ink-faint ml-1">{earned.length} de {badges.length}</span>
            </h2>
        ...
        <div className="flex-1 overflow-y-auto px-[22px] pt-2.5 pb-7 min-h-0 [overscroll-behavior:contain]">
          ...
          <div className="text-[11px] font-semibold tracking-[0.12em] uppercase text-ink-faint mt-[26px] mb-4">Por conseguir · {locked.length}</div>
          <div className="grid grid-cols-2 gap-x-2.5 gap-y-[18px] justify-items-center">
